			...
			<form method="POST" action="{{ route('donate:create') }}">
                ...
                <div class="grid grid-cols-4 gap-4 my-4">
                    @foreach ($banks as $bank)
                        <a href="{{ route('donate:billplz:create:bill', [$donation, $bank['code']]) }}" class="border rounded-lg px-auto py-3 text-center hover:bg-slate-400">{{ $bank['category'] }}{{ $bank['code'] }}</a>
                    @endforeach
                </div>
